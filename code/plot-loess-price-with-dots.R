...
p1 <- df_plot %>% 
  ggplot() +
  geom_point(aes(x = abertura_lances, y = win_bid_kg, shape=Plataforma),
             alpha = .08) +
...
ggsave(plot = p1,
       filename = 'results/plot-loess-price-with-dots.png',
       width = 5, height = 6)

# Versao sem título
p2 <- p1 +
  theme(plot.title = element_blank(),
        plot.subtitle = element_blank()) ; p2

# ggsave(plot = p2, width = 5, height = 6,
#        filename = 'results/plot-loess-price-no-title.png')